...
def default_corporate_policy(company_id: str = "default") -> TravelPolicy:
    return TravelPolicy(
        ...
        hotel_rules={
            "tier1": HotelRule(max_nightly_cny=Decimal("350"), city_tier="tier1"),
            "tier2": HotelRule(max_nightly_cny=Decimal("310"), city_tier="tier2"),
        },
        per_diem_cny={
            EmployeeGrade.STAFF: Decimal("100"),
            EmployeeGrade.MANAGER: Decimal("100"),
            EmployeeGrade.DIRECTOR: Decimal("100"),
            EmployeeGrade.EXECUTIVE: Decimal("100"),
        },
...
def evaluate_cabin_compliance(
    ...
    if _class_rank(chosen) > _class_rank(rule.max_class):
        warnings.append(
            f"所选舱位 {chosen.value} 超出职级允许上限 {rule.max_class.value}，需要特批或降级。"
        )
    return warnings
...
def evaluate_budget_and_approval(
    ...
    if request.budget_ceiling_cny and itinerary.total_estimated_cny > request.budget_ceiling_cny:
        warnings.append(
            f"行程预计 {itinerary.total_estimated_cny} CNY，超过个人填报上限 "
            f"{request.budget_ceiling_cny} CNY。"
        )
    if itinerary.total_estimated_cny > policy.requires_pre_approval_above_cny:
        warnings.append(
            f"金额超过公司事前审批线 {policy.requires_pre_approval_above_cny} CNY，需要提交 OA 审批。"
        )
    delta = (request.departure_date - date.today()).days
    if delta < 0:
        warnings.append("出发日期早于今天，请重新确认日期后再校验差标。")
    elif delta < policy.advance_booking_days_min:
        warnings.append(
            f"距离出发仅 {delta} 天，低于提前 {policy.advance_booking_days_min} 天预订要求，"
            "可能产生附加费用或需要补充说明。"
        )
    return warnings
...
def apply_policy_to_itinerary(
    ...
    *,
    ...
    if hotel_rule:
        merged.append(f"目的地酒店标准为每晚不超过 {hotel_rule.max_nightly_cny} CNY（{tier}）。")

...
def build_draft_itinerary(
    ...
    *,
    default_carrier: str = "示例供应商",
) -> Itinerary:
    """Build a deterministic draft itinerary for demo and policy checks."""
    ...
    if request.return_date and request.return_date > request.departure_date:
        ...
        ret_arrive = (ret_depart + flight_duration).astimezone(ret_tz)
        segments.append(
            Segment(
                segment_id=str(uuid.uuid4()),
    ...
    total = sum(_estimate_segment_cost_cny(s.from_city, s.to_city, s.transport) for s in segments)
...
def summarize_itinerary_text(itinerary: Itinerary) -> str:
    lines = [f"《{itinerary.title}》", f"预计总额：{itinerary.total_estimated_cny} CNY"]
    ...
    if itinerary.policy_warnings:
        lines.append("差标提示：" + "；".join(itinerary.policy_warnings))
    return "\n".join(lines)

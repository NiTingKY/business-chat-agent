...
async def health(request: Request) -> HealthStatus:
    ...
    if r is None:
        err = getattr(request.app.state, "redis_error", None)
        checks["redis"] = err is None
        if err:
            detail_parts.append(f"redis:{err}")
    else:
        try:
            await r.ping()
            checks["redis"] = True
    ...
    if milvus is None:
        ...
        backend = getattr(milvus, "backend", "milvus")
        if backend != "milvus":
            detail_parts.append(f"vector_store:{backend}")

    engine = getattr(request.app.state, "db_engine", None)
    if engine is None:
        err = getattr(request.app.state, "db_error", None)
        checks["database"] = err is None
        if err:
            detail_parts.append(f"db:{err}")
    else:
        try:
